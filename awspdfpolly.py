...
def main():
    ...
    if voice_option == "Male":
        voice_id = 'Matthew'  # Replace with actual male voice ID
    else:
        voice_id = 'Joanna'
    ...
    if st.button("Claude Output"):
        with st.spinner("Processing..."):
            ...
            st.write(res)
            audio_stream = synthesize_speech(res.replace("*", ""), voice_id)
            # st.audio(audio_stream, format='audio/mp3')
            # display(Audio(audio_stream, autoplay=True))
            audio_html = get_audio_html(audio_stream)
